...
class GameColors {
  ...
  static final ghosts = [Colors.red.shade500, Colors.pink.shade300, Colors.cyan.shade300, Colors.orange.shade500];
}
...
class _GamePageState extends State<GamePage> with TickerProviderStateMixin {
  ...
  List<List<int>> map = List.generate(rows, (r) => List.generate(cols, (c) => 2));
  ...
  // Ghosts state
  final List<Point<int>> ghosts = [const Point(10, 9), const Point(9, 10), const Point(10, 10), const Point(11, 10)];
  final List<Dir> ghostDirs = [Dir.left, Dir.right, Dir.up, Dir.down];

  @override
  void initState() {
    super.initState();
    _mouthAnimation = AnimationController(vsync: this, duration: const Duration(milliseconds: 250))
      ..repeat(reverse: true);
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: GameLevel.values
              .map<Widget>((level) => ElevatedButton(
                    onPressed: () {
  ...
  void _newMap() {
    map = _mapLayout.map<List<int>>((row) => row.split('').map<int>(int.parse).toList()).toList();
    ...
    ghosts.setAll(0, [const Point(10, 9), const Point(9, 10), const Point(10, 10), const Point(11, 10)]);
  }
  ...
  Point<int> _nextPoint(Point<int> p, Dir d) {
    ...
    switch (d) {
      case Dir.up: return Point(p.x, p.y - 1);
      case Dir.down: return Point(p.x, p.y + 1);
      case Dir.left: return Point(p.x - 1, p.y);
      case Dir.right: return Point(p.x + 1, p.y);
      case Dir.none: return p;
    }
  }
  ...
  void dispose() {
    ...
    if (isRunning && _timer.isActive) { // Check _timer.isActive before cancelling
      _timer.cancel();
    }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text('LEVEL: ${_level.name.toUpperCase()}', style: const TextStyle(fontSize: 14)),
        ],
      ),
    );
  }
  ...
  Widget _buildGameCanvas(BoxConstraints constraints) {
    ...
    final h = availableHeight.isFinite && availableHeight > 0 ? availableHeight : constraints.maxHeight * 0.7; 
    ...
    return GestureDetector(
      onPanUpdate: (d) {
        ...
        if (isRunning) { // Only allow direction changes if game is running
...
class _GamePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < map.length; r++) {
      for (int c = 0; c < map[r].length; c++) {
        ...
        if (map[r][c] == 1) { // Wall
          ...
          canvas.drawRRect(RRect.fromRectAndRadius(rect.deflate(2.0), const Radius.circular(4)), paint);
        } else if (map[r][c] == 0) { // Pellet
    ...
    // --- Draw Pac-Man ---
    final pacRect = Rect.fromLTWH(pac.x * tileSize, pac.y * tileSize, tileSize, tileSize);
    ...
    switch (pacDir) {
      case Dir.right: startAngle = mouthAngle; break;
      case Dir.left: startAngle = pi + mouthAngle; break;
      case Dir.up: startAngle = -pi / 2 + mouthAngle; break;
      case Dir.down: startAngle = pi / 2 + mouthAngle; break;
      case Dir.none: startAngle = mouthAngle; break;
    }
    final sweepAngle = 2 * pi - (mouthAngle * 2);
    canvas.drawArc(pacRect.deflate(tileSize * 0.1), startAngle, sweepAngle, true, paint);

    // --- Draw Ghosts ---
    for (int i = 0; i < ghosts.length; i++) {
      final g = ghosts[i];
      final gr = Rect.fromLTWH(g.x * tileSize, g.y * tileSize, tileSize, tileSize).deflate(tileSize * 0.1);
      paint.color = GameColors.ghosts[i % GameColors.ghosts.length];
      
      // Ghost body shape
      final path = Path();
      path.moveTo(gr.left, gr.center.dy);
      path.arcTo(Rect.fromCircle(center: Offset(gr.center.dx, gr.top + gr.height / 4), radius: gr.width/2), pi, pi, false);
      path.lineTo(gr.right, gr.center.dy);
      // Wavy bottom
      path.quadraticBezierTo(gr.right - gr.width / 4, gr.bottom + gr.height/8, gr.center.dx, gr.bottom);
      path.quadraticBezierTo(gr.left + gr.width / 4, gr.bottom - gr.height/8, gr.left, gr.center.dy);
      path.close();
      ...
      canvas.drawCircle(gr.center.translate(-eyeOffsetX, eyeOffsetY), eyeRadius, paint);
      paint.color = Colors.black;
      canvas.drawCircle(gr.center.translate(-eyeOffsetX, eyeOffsetY), pupilRadius, paint);
